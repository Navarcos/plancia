...
import OptionsButton from "./options/MoreOptionsButton";
...
const DepTable: React.FC<DepTableProps> = () => {
  ...
  const rowsNumber = isSmallScreen ? 20 : is4KScreen ? 30 : 24;
  ...
  function fetchData(namespace: string, name: string) {
    ResourceApi.getDeployments(namespace!, name!)
      .then((data) => {
        console.log(data);
        setRows(data.items);
  ...
  return (
    <Container maxWidth={false} className="dp-container">
      <Box className="dp-main-box">
        <Paper className="dp-title-paper">
          <CustomTypography variant="h6">Deployments</CustomTypography>
          <motion.div {...shakeAnimation}>
            <Tooltip title="Click on row to be directed to selected Deployment overview.">
              <IconButton size="small">
                <InfoIcon className="dp-tooltip-icon" />
              </IconButton>
            </Tooltip>
          </motion.div>
        </Paper>

        <Paper className="dp-filter-paper">
        ...
        <Paper className="pagination-list-table-container">
          <TableContainer component={Paper} className="dp-list-table-container">
            <Table aria-label="collapsible table">
              <TableHead>
                <TableRow>
                  {!isSmallScreen && (
                    <TableCell className="dp-table-cell-1" padding={"none"} />
                  )}
                  <TableCell className="dp-table-cells">Name</TableCell>
                  {!isSmallScreen && (
                    <TableCell className="dp-table-cells">Namespace</TableCell>
                  )}
                  <TableCell className="dp-table-cells">Age</TableCell>
                  <TableCell className="dp-table-cells">
                    Resource Version
                  </TableCell>
        ...
        {selectedRow && (
          <Box mt={2} className="dp-dialog-box">
            <Dialog
              open={dialogOpen}
              ...
              className="dp-dialog"
            >
              <DialogTitle className="dp-dialog-title">
